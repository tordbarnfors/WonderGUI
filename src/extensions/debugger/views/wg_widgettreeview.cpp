...
namespace wg
{
	...
	const TypeInfo WidgetTreeView::TYPEINFO = { "WidgetTreeView", &InspectorView::TYPEINFO };


	//____ constructor _____________________________________________________________

	WidgetTreeView::WidgetTreeView(const DebugTheme& theme, IDebugContext * pContext, Widget * pRoot ) : InspectorView( theme, pContext )
	{
		...
		if( pRoot )
			m_pSelectCapsule->slot = _generateInfoTree(theme, pRoot );
		else
		...
		m_routeIdForSelect = Base::msgRouter()->addRoute(m_pSelectCapsule, MsgType::Selected, [this](Msg* pMsg) {
			...
			if (pSelected)
			{
				m_pContext->objectSelected(pSelected,this);
	...
	Widget_p WidgetTreeView::_generateInfoTree(const DebugTheme& theme, Widget* pWidget, int indentation)
	{
		if (pWidget->isContainer())
		{
			auto pDrawer = dbgkit::TreeListDrawer::create( WGOVR(theme.listEntryDrawer, 
				_.buttonOfs.x += pts(indentation * 16)
			));

			auto pNameDisplay = TextDisplay::create(WGOVR(theme.listEntryLabel, _.display.text = pWidget->typeInfo().className));

			auto pEntry = dbgkit::TreeListEntry::create(WGOVR(theme.selectableListEntryCapsule,
			...
			while (pChild)
			{
				pContent->slots.pushBack(_generateInfoTree(theme, pChild, indentation + 1) );
				pChild = pChild->nextSibling();
			}
