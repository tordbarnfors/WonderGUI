...
//			Delta8_int = 7			// 8 bit delta pixel values.
		...
		struct DataInfo
		{
			...
			uint8_t		chunkComp;		// Read only. Set by decoder. SPX-compression used for this specific chunk in old, deprecated stream format.
		};
		...
		static DataInfo readDataInfo(const uint8_t* pChunkData);
		static int		loadDecompressData( DataInfo& dataInfo, uint8_t * pBuffer, const uint8_t * pSource, int bytes );
