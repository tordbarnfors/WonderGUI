...
namespace wg
{
	...
	StreamBackend_p StreamBackend::create(const Blueprint& blueprint)
	{
		return new StreamBackend(blueprint);
	}
	...
	StreamBackend::StreamBackend(const Blueprint& bp)
	{
		...
		m_pCommandCompressor 	= bp.commandCompressor;
		m_pObjectCompressor		= bp.objectCompressor;
		m_pRectCompressor		= bp.rectCompressor;		// Both for render rects and update rects.
		m_pTransformCompressor	= bp.transformCompressor;
	...
	void StreamBackend::processCommands( const uint16_t* pBeg, const uint16_t * pEnd, int version)
	{
	...
	void StreamBackend::_compressSplitAndEncode( StreamEncoder * pEncoder, GfxStream::ChunkId chunkType, uint16_t objectID, Compressor * pCompressor, const void * _pBeg, const void * _pEnd )
	{
		...
		uint32_t compression = Util::makeEndianSpecificToken( 'N','O','N','E' );
		...
		if( pCompressor )
		{
			...
			workBufferSize = memStackAllocated;
			compression = pCompressor->idToken();
			dataStart = workBufferSize - size;
		}
		...
		GfxBase::memStackFree(memStackAllocated);
	}
