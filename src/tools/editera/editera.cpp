...
void Editera::closeWindow(wapp::Window* _pWindow)
{
...
{
	if( !m_pAPI->initDefaultWidgetKit() )
		return false;

	m_pEditorLayout = BasicTextLayout::create({ .tabWidth = 4 });

	m_pEditorStyle = wkit::TextStyles::Mono;
...
bool Editera::createEditorWindow( const std::string& windowTitle, const std::string& path )
{
	std::string title;
...
std::string Editera::_createWindowTitle( const std::string& path )
{
	std::string title;
