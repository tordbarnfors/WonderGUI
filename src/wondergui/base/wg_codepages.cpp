...
namespace wg
{
	...
	const static uint16_t latin1[128] = {	0x0080, 0x0081, 0x0082, 0x0083, 0x0084, 0x0085, 0x0086, 0x0087, 0x0088, 0x0089, 0x008A, 0x008B, 0x008C, 0x008D, 0x008E, 0x008F,
	...
	const static uint16_t win1250[128] = { 	0x20AC, 0xFFFD, 0x201A, 0xFFFD, 0x201E, 0x2026, 0x2020, 0x2021, 0xFFFD, 0x2030, 0x0160, 0x2039, 0x015A, 0x0164, 0x017D, 0x0179,
	...
	const static uint16_t win1251[128] = {	0x0402, 0x0403, 0x201A, 0x0453, 0x201E, 0x2026, 0x2020, 0x2021, 0x20AC, 0x2030, 0x0409, 0x2039, 0x040A, 0x040C, 0x040B, 0x040F,
	...
	const static uint16_t win1252[128] = {	0x20AC, 0xFFFD, 0x201A, 0x0192, 0x201E, 0x2026, 0x2020, 0x2021, 0x02C6, 0x2030, 0x0160, 0x2039, 0x0152, 0xFFFD, 0x017D, 0xFFFD,
	...
	const static uint16_t win1253[128] = {	0x20AC, 0xFFFD, 0x201A, 0x0192, 0x201E, 0x2026, 0x2020, 0x2021, 0xFFFD, 0x2030, 0xFFFD, 0x2039, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD,
	...
	const static uint16_t win1254[128] = {	0x20AC, 0xFFFD, 0x201A, 0x0192, 0x201E, 0x2026, 0x2020, 0x2021, 0x02C6, 0x2030, 0x0160, 0x2039, 0x0152, 0xFFFD, 0xFFFD, 0xFFFD,
	...
	const static uint16_t win1255[128] = {	0x20AC, 0xFFFD, 0x201A, 0x0192, 0x201E, 0x2026, 0x2020, 0x2021, 0x02C6, 0x2030, 0xFFFD, 0x2039, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD,
	...
	const static uint16_t win1256[128] = { 	0x20AC, 0x067E, 0x201A, 0x0192, 0x201E, 0x2026, 0x2020, 0x2021, 0x02C6, 0x2030, 0x0679, 0x2039, 0x0152, 0x0686, 0x0698, 0x0688,
	...
	const static uint16_t win1257[128] = { 	0x20AC, 0xFFFD, 0x201A, 0xFFFD, 0x201E, 0x2026, 0x2020, 0x2021, 0xFFFD, 0x2030, 0xFFFD, 0x2039, 0xFFFD, 0x00A8, 0x02C7, 0x00B8,
	...
	const static uint16_t win1258[128] = { 	0x20AC, 0xFFFD, 0x201A, 0x0192, 0x201E, 0x2026, 0x2020, 0x2021, 0x02C6, 0x2030, 0xFFFD, 0x2039, 0x0152, 0xFFFD, 0xFFFD, 0xFFFD,
	...
	const static uint16_t win874[128] =  {	0x20AC, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD, 0x2026, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD,
	...
	const static uint16_t mac_roman[128] = {0x00C4, 0x00C5, 0x00C7, 0x00C9, 0x00D1, 0x00D6, 0x00DC, 0x00E1,  // 0x80-0x87
	...
	const static uint16_t atari_st[128] = {	0x00C7, 0x00FC, 0x00E9, 0x00E2, 0x00E4, 0x00E0, 0x00E5, 0x00E7,  // 0x80-0x87
											...
											0x05DF, 0x05DA, 0x05DD, 0x05E3, 0x05E5, 0x00A7, 0x2038, 0x221E,  // 0xD8-0xDF
	...
	const static uint16_t riscos[128] = {		// 0x80-0x9F: RISC OS specific region
										0x20AC, // 0x80 - Euro sign
										0x0081, // 0x81 - Undefined
										0x201A, // 0x82 - Single low-9 quotation mark
										0xFFFD, // 0x83 - Resize window icon (replacement char)
										0xFFFD, // 0x84 - Close window icon (replacement char)
										0x2026, // 0x85 - Horizontal ellipsis
										0x2020, // 0x86 - Dagger
										0xFFFD, // 0x87 - Subscript 8 + Superscript 7 (replacement char)
										0xFFFD, // 0x88 - Left scroll arrow (replacement char)
										0xFFFD, // 0x89 - Right scroll arrow (replacement char)
										0xFFFD, // 0x8A - Up scroll arrow (replacement char)
										0xFFFD, // 0x8B - Down scroll arrow (replacement char)
	...
	// DOS Code Page 437 (IBM PC US)

	const static uint16_t dos_437[128] = {
									// 0x80-0x8F: International characters (Western European accented letters)
									...
									// 0x90-0x9F: More international characters + currency symbols
									...
									// 0xA0-0xAF: Spanish/Portuguese characters + fractions + quotation marks
									0x00E1, 0x00ED, 0x00F3, 0x00FA, 0x00F1, 0x00D1, 0x00AA, 0x00BA,  // á í ó ú ñ Ñ ª º
									0x00BF, 0x2310, 0x00AC, 0x00BD, 0x00BC, 0x00A1, 0x00AB, 0x00BB,  // ¿ ⌐ ¬ ½ ¼ ¡ « »

									// 0xB0-0xBF: Block/shade characters + box drawing (light single lines + mixed)
									0x2591, 0x2592, 0x2593, 0x2502, 0x2524, 0x2561, 0x2562, 0x2556,  // ░ ▒ ▓ │ ┤ ╡ ╢ ╖
									0x2555, 0x2563, 0x2551, 0x2557, 0x255D, 0x255C, 0x255B, 0x2510,  // ╕ ╣ ║ ╗ ╝ ╜ ╛ ┐

									// 0xC0-0xCF: Box drawing characters (light single + double lines)
									0x2514, 0x2534, 0x252C, 0x251C, 0x2500, 0x253C, 0x255E, 0x255F,  // └ ┴ ┬ ├ ─ ┼ ╞ ╟
									0x255A, 0x2554, 0x2569, 0x2566, 0x2560, 0x2550, 0x256C, 0x2567,  // ╚ ╔ ╩ ╦ ╠ ═ ╬ ╧

									// 0xD0-0xDF: More box drawing + block characters
									0x2568, 0x2564, 0x2565, 0x2559, 0x2558, 0x2552, 0x2553, 0x256B,  // ╨ ╤ ╥ ╙ ╘ ╒ ╓ ╫
									0x256A, 0x2518, 0x250C, 0x2588, 0x2584, 0x258C, 0x2590, 0x2580,  // ╪ ┘ ┌ █ ▄ ▌ ▐ ▀

									// 0xE0-0xEF: Greek letters + mathematical symbols
									0x03B1, 0x00DF, 0x0393, 0x03C0, 0x03A3, 0x03C3, 0x00B5, 0x03C4,  // α ß Γ π Σ σ µ τ
									0x03A6, 0x0398, 0x03A9, 0x03B4, 0x221E, 0x03C6, 0x03B5, 0x2229,  // Φ Θ Ω δ ∞ φ ε ∩

									// 0xF0-0xFF: Mathematical symbols + miscellaneous
									0x2261, 0x00B1, 0x2265, 0x2264, 0x2320, 0x2321, 0x00F7, 0x2248,  // ≡ ± ≥ ≤ ⌠ ⌡ ÷ ≈
									0x00B0, 0x2219, 0x00B7, 0x221A, 0x207F, 0x00B2, 0x25A0, 0x00A0   // ° ∙ · √ ⁿ ² ■ [NBSP]
	};

	// Code Page 850 (DOS Latin-1)

	const uint16_t dos_850[128] = {
									// 0x80-0x8F: Western European accented letters
									0x00C7, 0x00FC, 0x00E9, 0x00E2, 0x00E4, 0x00E0, 0x00E5, 0x00E7,  // Ç ü é â ä à å ç
									0x00EA, 0x00EB, 0x00E8, 0x00EF, 0x00EE, 0x00EC, 0x00C4, 0x00C5,  // ê ë è ï î ì Ä Å

									// 0x90-0x9F: More Western European characters + currency
									0x00C9, 0x00E6, 0x00C6, 0x00F4, 0x00F6, 0x00F2, 0x00FB, 0x00F9,  // É æ Æ ô ö ò û ù
									0x00FF, 0x00D6, 0x00DC, 0x00F8, 0x00A3, 0x00D8, 0x00D7, 0x0192,  // ÿ Ö Ü ø £ Ø × ƒ

									// 0xA0-0xAF: Spanish/Portuguese + symbols + quotation marks
									0x00E1, 0x00ED, 0x00F3, 0x00FA, 0x00F1, 0x00D1, 0x00AA, 0x00BA,  // á í ó ú ñ Ñ ª º
									0x00BF, 0x00AE, 0x00AC, 0x00BD, 0x00BC, 0x00A1, 0x00AB, 0x00BB,  // ¿ ® ¬ ½ ¼ ¡ « »

									// 0xB0-0xBF: Block characters + box drawing (single lines) + more accented letters
									0x2591, 0x2592, 0x2593, 0x2502, 0x2524, 0x00C1, 0x00C2, 0x00C0,  // ░ ▒ ▓ │ ┤ Á Â À
									0x00A9, 0x2563, 0x2551, 0x2557, 0x255D, 0x00A2, 0x00A5, 0x2510,  // © ╣ ║ ╗ ╝ ¢ ¥ ┐

									// 0xC0-0xCF: Box drawing (single + double lines) + accented letters
									0x2514, 0x2534, 0x252C, 0x251C, 0x2500, 0x253C, 0x00E3, 0x00C3,  // └ ┴ ┬ ├ ─ ┼ ã Ã
									0x255A, 0x2554, 0x2569, 0x2566, 0x2560, 0x2550, 0x256C, 0x00A4,  // ╚ ╔ ╩ ╦ ╠ ═ ╬ ¤

									// 0xD0-0xDF: More accented letters + block characters
									0x00F0, 0x00D0, 0x00CA, 0x00CB, 0x00C8, 0x0131, 0x00CD, 0x00CE,  // ð Ð Ê Ë È ı Í Î
									0x00CF, 0x2518, 0x250C, 0x2588, 0x2584, 0x00A6, 0x00CC, 0x2580,  // Ï ┘ ┌ █ ▄ ¦ Ì ▀

									// 0xE0-0xEF: More Western European accented letters
									0x00D3, 0x00DF, 0x00D4, 0x00D2, 0x00F5, 0x00D5, 0x00B5, 0x00FE,  // Ó ß Ô Ò õ Õ µ þ
									0x00DE, 0x00DA, 0x00DB, 0x00D9, 0x00FD, 0x00DD, 0x00AF, 0x00B4,  // Þ Ú Û Ù ý Ý ¯ ´

									// 0xF0-0xFF: Mathematical symbols + miscellaneous
									0x00AD, 0x00B1, 0x2017, 0x00BE, 0x00B6, 0x00A7, 0x00F7, 0x00B8,  // ­ ± ‗ ¾ ¶ § ÷ ¸
									0x00B0, 0x00A8, 0x00B7, 0x00B9, 0x00B3, 0x00B2, 0x25A0, 0x00A0   // ° ¨ · ¹ ³ ² ■ [NBSP]
	};

	const uint16_t * CodePages::LookupTable[CodePage_size] = {
		nullptr,
		latin1,
		dos_437,
		dos_850,
