...
namespace wg
{
	...
	const TypeInfo WaveChart::TYPEINFO = { "WaveChart", &Widget::TYPEINFO };


	//____ constructor ____________________________________________________________

	WaveChart::WaveChart() : entries(this)
	{
	}
	...
	const TypeInfo& WaveChart::typeInfo(void) const
	{
		return TYPEINFO;
	}
	...
	void WaveChart::_startedOrEndedTransition()
	{
	...
	void WaveChart::_entryVisibilityChanged(WaveChartEntry* pWaveChartEntry)
	{
		// We need to render the area no matter if graph appeared or disappeared.

		_requestRenderWaveChartEntry(pWaveChartEntry, pWaveChartEntry->m_begin, pWaveChartEntry->m_end);

		// If we turned visible we might have a waveform that needs refresh.
		// (we don't perform refresh while hidden)

		if (pWaveChartEntry->m_bVisible && pWaveChartEntry->m_pWaveform && (pWaveChartEntry->m_bSamplesChanged || pWaveChartEntry->m_bColorsChanged))
			_waveformNeedsRefresh(pWaveChartEntry, false, pWaveChartEntry->m_bSamplesChanged, pWaveChartEntry->m_bColorsChanged);
	}


	//____ _setWaveChartEntryRange() __________________________________________________

	void WaveChart::_setWaveChartEntryRange(WaveChartEntry* pWaveChartEntry, float begin, float end)
	{
		float leftmost = std::min(begin, pWaveChartEntry->m_begin);
		float rightmost = std::max(end, pWaveChartEntry->m_end);

		pWaveChartEntry->m_begin = begin;
		pWaveChartEntry->m_end = end;

		_requestRenderWaveChartEntry( pWaveChartEntry, leftmost, rightmost );
		_waveformNeedsRefresh(pWaveChartEntry, true, false, false);
	}

	//____ _didAddEntries() ___________________________________________________

	void WaveChart::_didAddEntries(WaveChartEntry* pEntry, int nb)
	{
		for (int i = 0; i < nb; i++)
		{
	...
	void WaveChart::_didMoveEntries(WaveChartEntry* pFrom, WaveChartEntry* pTo, int nb)
	{
		for (int i = 0; i < nb; i++)
		{
			WaveChartEntry* p = pTo + i;
			_requestRenderWaveChartEntry(p, p->m_begin, p->m_end);
		}
	...
	void WaveChart::_willEraseEntries(WaveChartEntry* pEntry, int nb)
	{
		for (int i = 0; i < nb; i++)
		{
			WaveChartEntry* p = pEntry + i;
			_requestRenderWaveChartEntry(p, p->m_begin, p->m_end);
		}
	}

	//____ _requestRenderWaveChartEntry() ______________________________________________

	void WaveChart::_requestRenderWaveChartEntry(WaveChartEntry* pWaveChartEntry, float leftmost, float rightmost)
	{
		//TODO: Take max/min sample values into account after they are maintained.

		RectSPX dirt;

		if( pWaveChartEntry->m_bAxisSwapped )
		{
		...
		pts outlineThickness = std::max(pWaveChartEntry->m_topOutlineThickness, pWaveChartEntry->m_bottomOutlineThickness);
	...
	void WaveChart::_waveformNeedsRefresh(WaveChartEntry * pWaveChartEntry, bool bGeo, bool bSamples, bool bColors)
	{
		if (bGeo)
		{
			pWaveChartEntry->m_pWaveform = nullptr;
		}
		else
		{
			if (bSamples)
				pWaveChartEntry->m_bSamplesChanged = true;

			if (bColors)
				pWaveChartEntry->m_bColorsChanged = true;
		}

		if (pWaveChartEntry->m_bVisible && !m_bPreRenderRequested)
			m_bPreRenderRequested = _requestPreRenderCall();
	}

	//____ _fullRefreshOfChart() _______________________________________

	void WaveChart::_fullRefreshOfChart()
	{
		for (auto& graph : entries)
	...
	void WaveChart::_renderCharts(GfxDevice* pDevice, const RectSPX& canvas)
	{
	...
	void WaveChart::_update(int microPassed, int64_t microsecTimestamp)
	{
	...
	//____ _updateWaveChartEntrys() ______________________________________________________

	void WaveChart::_updateWaveChartEntrys()
	{
		for (auto& graph : entries)
		{
			if (graph.m_bVisible)
			{
				...
				if (bNeedsRendering )
					_requestRenderWaveChartEntry(&graph, graph.m_begin, graph.m_end);
			}
	...
	void WaveChart::_updateWaveformEdge(Waveform* pWaveform, bool bTopEdge, int nSamples, float* pSamples, bool bAxisSwapped )
	{
	...
	RectSPX WaveChart::_entryRangeToRect(float begin, float end, bool axisSwapped) const
	{
		RectSPX rect;
	...
	void WaveChart::_preRender()
	{
		_updateWaveChartEntrys();
		m_bPreRenderRequested = false;
	}

	//____ WaveChartEntry::WaveChartEntry() _______________________________________________________

	WaveChartEntry::WaveChartEntry(const Blueprint& bp)
	{
		m_id = bp.id;
	...
	bool WaveChartEntry::setColors(HiColor fill, HiColor outline, ColorTransition* pTransition)
	{
	...
	bool WaveChartEntry::setGradients(Gradient fill, Gradient outline, ColorTransition* pTransition)
	{
		if( !fill.isValid() || !outline.isValid() )
			return false;
	...
	//____ WaveChartEntry::setOutlineThickness() ___________________________________

	bool WaveChartEntry::setOutlineThickness( pts topOutline, pts bottomOutline )
	{
	...
	void WaveChartEntry::setRange(float begin, float end)
	{
		limit(begin, 0.f, 1.f);
		limit(end, begin, 1.f);
		...
		m_pDisplay->_setWaveChartEntryRange(this, begin, end);
	}

	//____ setTopSamples() ____________________________________________________

	void WaveChartEntry::setTopSamples(int nSamples, const float* pSamples)
	{
		if (nSamples == 0)
		{
	...
	void WaveChartEntry::setBottomSamples(int nSamples, const float* pSamples)
	{
		if (nSamples == 0)
		{
	...
	bool WaveChartEntry::transitionSamples(ValueTransition* pTransition, int nTopSamples, const float* pNewTopSamples, int nBottomSamples, const float* pNewBottomSamples)
	{
	...
	void WaveChartEntry::setVisible(bool bVisible)
	{
	...
	void WaveChartEntry::setFlip( GfxFlip flip )
	{
		if( flip != m_flip )
		{
			m_flip = flip;
	...
	std::tuple<int, const float*>  WaveChartEntry::topSamples() const 
	{ 
	...
	std::tuple<int, const float*>  WaveChartEntry::bottomSamples() const 
	{ 
	...
	void WaveChartEntry::_endSampleTransition()
	{
	...
	void WaveChartEntry::_endColorTransition()
	{
